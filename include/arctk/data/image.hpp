    ...
    namespace data //! data namespace
    {
        ...
        class Image
        {
          ...
          public:
            ...
            virtual inline void save(const std::string& path_, double(const* scale_)(double)) const
              noexcept = 0; //!< Save the image data using a value scaling function. Pixel values are normalised then scaled using the given scaling function. Pixel values are
            ...
            virtual inline void save(const std::string& path_, vec3(const* map_)(double)) const noexcept = 0; //!< Save the image data using a colour mapping function. Pixel values are normalised but not scaled. Pixel values are converted to a colour
                                                                                                              //!< using the given mapping function. @param  path_   Path to the output file. @param  map_ Function used to map scaled values to a colour.
            virtual inline void save(const std::string& path_, double(const* scale_)(double), vec3(const* map_)(double)) const
