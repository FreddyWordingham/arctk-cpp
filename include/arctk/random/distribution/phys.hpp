    ...
    namespace random //! utility namespace
    {
        namespace distribution //! distribution namespace
        {
            namespace phys //! physics namespace
            {
                ...
                inline vec3 isotropic(Generator* const rng_) noexcept
                {
                    const double theta = arc::random::distribution::uniform<double>(rng_, 0.0, 2.0 * arc::constant::PI);
                    const double z     = arc::random::distribution::uniform<double>(rng_, -1.0, 1.0);
