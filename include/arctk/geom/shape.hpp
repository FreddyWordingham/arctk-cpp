    ...
    namespace geom //! geometry namespace
    {
        ...
        class Shape
        {
          ...
          public:
            ...
            virtual double area() const noexcept   = 0; //!< Get the total surface area of the shape.    @return Total surface area of the shape.
            virtual double vol() const noexcept    = 0; //!< Get the total volume of the shape.    @return Total volume of the shape.
            virtual bool   closed() const noexcept = 0; //!< Get the topology of the shape.    @return True if the surface of the shape forms a bounded volume.

            //  -- Emission --
            virtual vec3 random_pos(random::Generator* rng_) const noexcept = 0; //!< Generate a random position on the surface of the shape. @param  rng_    Random number generator.    @return Randomly generated point on the surface of the shape.
            virtual std::pair<vec3, vec3> random_pos_and_norm(random::Generator* rng_) const
            ...
            //  -- Intersection --
            virtual bool intersect_surf(const shape::Aabb& aabb_) const noexcept = 0; //!< Determine if an intersection occurs between the shape's surface and an axis-aligned bounding box.   @param  aabb_   Axis-aligned bounding box to test. @return
                                                                                      //!< True if the shape's surface intersects with the axis-aligned bounding box.
            virtual bool intersect_vol(const shape::Aabb& aabb_) const noexcept = 0;  //!< Determine if an intersection occurs between the shape's volume and an axis-aligned bounding box.    @param  aabb_   Axis-aligned bounding box to test.  @return
